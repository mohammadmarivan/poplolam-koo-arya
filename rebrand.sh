#!/usr/bin/env bash
# AppForge rebrand — poolamkoo
# source: doxigo/muchToman  license: MIT
set -euo pipefail
cd "$(dirname "$0")"

echo "rebrand چقدر تومن -> poolamkoo"
echo "package com.doxigo.muchtoman -> com.poolamkoo"
...
if command -v python3 >/dev/null 2>&1; then
  ...
  echo "Python is required for rebrand.sh"
  exit 1
fi
...
echo
echo "done. next:"
echo "  git add -A && git commit -m \"rebrand: poolamkoo\""
echo "  git remote rename origin upstream || true"
echo "  git remote add origin https://github.com/mohammadmarivan/poplolam-koo-arya.git"
